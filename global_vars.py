...
def last_update():
    return pd.read_sql("select max(update_id) from  raw_updates", con=sql_connection).values[0,0]


@create_module_property_from_init_func
def last_update_handled():
    return pd.read_sql("select max(update_id) from  update_handle_log where log='handled'", con=sql_connection).values[0,0]


@create_module_property_from_init_func
def last_update_telegram_id():
    df = pd.read_sql("select update_telegram_id from  raw_updates where update_id=?", con=sql_connection, params=(int(last_update.get()),))
    if df.empty:
        return -1
    return df.values[0,0]


options_df = pd.read_sql("select * from  options", con=sql_connection)
